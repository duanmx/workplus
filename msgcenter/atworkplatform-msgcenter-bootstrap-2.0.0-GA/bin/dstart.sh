...
if [ -n "$PIDS" ]; then
    echo "ERROR: msgcenter has been started!"
    echo "PID: $PIDS"
    exit 1
fi
...
STDOUT_FILE=$LOGS_DIR/msgcenter.log
...
echo -e "starting msgcenter ...\c"
nohup java $JAVA_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -cp $ETC_DIR:$LIB_JARS io.atworkplatform.msgcenter.bootstrap.MessageCenterBootstrap >> $STDOUT_FILE 2>&1 &
