...
if [ -n "$PIDS" ]; then
    echo "ERROR: sessioncenter has been started!"
    echo "PID: $PIDS"
    exit 1
fi
...
STDOUT_FILE=$LOGS_DIR/sessioncenter.log
...
echo -e "starting sessioncenter ...\c"
nohup java $JAVA_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -cp $ETC_DIR:$LIB_JARS io.atworkplatform.sessioncenter.bootstrap.SessionCenterBootstrap >> $STDOUT_FILE 2>&1 &
