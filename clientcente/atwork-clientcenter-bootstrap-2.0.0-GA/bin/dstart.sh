...
if [ -n "$PIDS" ]; then
    echo "ERROR: atwork-clientcenter-bootstrap.iml has been started!"
    echo "PID: $PIDS"
    exit 1
fi
...
STDOUT_FILE=$LOGS_DIR/atwork-clientcenter-bootstrap.log
...
echo -e "starting clientcenter ...\c"
nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -cp $ETC_DIR:$LIB_JARS io.atwork.clientcenter.bootstrap.ClientCenterBootstrap >> $STDOUT_FILE 2>&1 &
