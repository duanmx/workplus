...
if [ -n "$PIDS" ]; then
    echo "ERROR: appmarket has been started!"
    echo "PID: $PIDS"
    exit 1
fi
...
STDOUT_FILE=$LOGS_DIR/appmarket.log
...
#nohup
echo -e "starting appmarket ...\c"
 java $JAVA_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -cp $ETC_DIR:$LIB_JARS io.atwork.appmarket.bootstrap.AppMarketBootstrap
