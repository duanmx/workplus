...
if [ -n "$PIDS" ]; then
    echo "ERROR: atwork-organization-bootstrap has been started!"
    echo "PID: $PIDS"
    exit 1
fi
...
STDOUT_FILE=$LOGS_DIR/atwork-organization-bootstrap.log
...
echo -e "starting atwork-organization-bootstrap ...\c"
nohup java $JAVA_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -cp $ETC_DIR:$LIB_JARS io.atwork.organization.bootstrap.OrganizationBootstrap >> $STDOUT_FILE 2>&1 &
