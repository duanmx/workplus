...
if [ -n "$PIDS" ]; then
    echo "ERROR: schdcenter has been started!"
    echo "PID: $PIDS"
    exit 1
fi
...
STDOUT_FILE=$LOGS_DIR/schdcenter.log
...
echo -e "starting the schdcenter ...\c"
nohup java $JAVA_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -cp $ETC_DIR:$LIB_JARS io.atworkplatform.schdcenter.bootstrap.SchdCenterBootstrap >> $STDOUT_FILE 2>&1 &
