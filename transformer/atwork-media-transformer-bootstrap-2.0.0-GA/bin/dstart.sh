...
if [ -n "$PIDS" ]; then
    echo "ERROR: MediaTransformerBootstrap has been started!"
    echo "PID: $PIDS"
    exit 1
fi
...
STDOUT_FILE=$LOGS_DIR/MediaTransformerBootstrap.log
...
nohup java $JAVA_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -cp $ETC_DIR:$LIB_JARS io.atwork.media.transformer.bootstrap.MediaTransformerBootstrap >> $STDOUT_FILE 2>&1 &
