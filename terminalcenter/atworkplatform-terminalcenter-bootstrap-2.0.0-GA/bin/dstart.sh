...
if [ -n "$PIDS" ]; then
    echo "ERROR: terminalcenter has been started!"
    echo "PID: $PIDS"
    exit 1
fi
...
STDOUT_FILE=$LOGS_DIR/terminalcenter.log
...
echo -e "starting terminalcenter ...\c"
nohup java $JAVA_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -cp $ETC_DIR:$LIB_JARS io.atworkplatform.terminalcenter.bootstrap.TerminalCenterBootstrap >> $STDOUT_FILE 2>&1 &
