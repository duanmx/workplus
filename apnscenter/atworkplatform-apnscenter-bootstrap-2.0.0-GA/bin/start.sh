...
if [ -n "$PIDS" ]; then
    echo "ERROR: apnscenter has been started!"
    echo "PID: $PIDS"
    exit 1
fi
...
STDOUT_FILE=$LOGS_DIR/apnscenter.log
...
echo -e "starting the apnscenter ...\c"
java $JAVA_OPTS $JAVA_JMX_OPTS -cp $ETC_DIR:$LIB_JARS io.atworkplatform.apnscenter.bootstrap.ApnsCenterBootstrap 