...
if [ -n "$PIDS" ]; then
    echo "ERROR: usercenter has been started!"
    echo "PID: $PIDS"
    exit 1
fi
...
STDOUT_FILE=$LOGS_DIR/usercenter.log
...
echo -e "starting usercenter ...\c"
nohup java $JAVA_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -cp $ETC_DIR:$LIB_JARS io.atworkplatform.usercenter.bootstrap.UserCenterBootstrap >> $STDOUT_FILE 2>&1 &
