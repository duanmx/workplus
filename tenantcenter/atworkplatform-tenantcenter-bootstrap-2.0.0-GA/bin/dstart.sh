...
if [ -n "$PIDS" ]; then
    echo "ERROR: tenantcenter has been started!"
    echo "PID: $PIDS"
    exit 1
fi
...
STDOUT_FILE=$LOGS_DIR/tenantcenter.log
...
echo -e "starting tenantcenter ...\c"
nohup java $JAVA_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -cp $ETC_DIR:$LIB_JARS io.atworkplatform.tenantcenter.bootstrap.TenantCenterBootstrap >> $STDOUT_FILE 2>&1 &
